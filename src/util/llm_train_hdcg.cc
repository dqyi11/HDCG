...
evaluate_cv( const Grounding* grounding,
  ...
  } else if ( dynamic_cast< const h2sl_hdcg::Spatial_Function* >( grounding ) != NULL ){ 
    const h2sl_hdcg::Spatial_Function* spatial_function_grounding = dynamic_cast< const h2sl_hdcg::Spatial_Function* >( grounding );
    cv = CV_FALSE;
    for( unsigned int i = 0; i < groundingSet->groundings().size(); i++ ){
...
main( int argc,
  ...
  for( unsigned int i = 0; i < args.threads_arg; i++ ){
    feature_sets.push_back( new h2sl_hdcg::Feature_Set() );
